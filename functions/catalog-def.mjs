...
export const CATALOG = {
  ...
  personalDistributions: {
    label: "代表個人の分配金（投資信託）",
    holds: ["銘柄ごとの年間分配 annual"],
    amountField: "annual", keyFields: ["owner", "year"],
    notHere: "法人の収入ではない。役員報酬0円の土台になっている個人の収入",
    relatedTo: ["personalAssets"],
  },
  revenue: { label: "宿泊の売上（月次・棟別）", holds: ["売上 revenue と手取り payout", "OTA手数料・清掃・宿泊税・管理料の内訳", "稼働 occ・ADR・組数"], amountField: "payout", keyFields: ["prop", "month"], notHere: "経費は無い（utilities / recurringCosts / taxes / insurance を見る）", relatedTo: ["bookingDaily"] },
