...
const TOOLS: FunctionDeclaration[] = [
  ...
  },
  {
    ...
    description: "上の道具に無いデータの最後の手段。agency DB の台帳を生のまま読む（読み取り専用）。" +
      ...
      "templates / alertLogs / unmatched / heartbeats / competitorObs（AirDNA市場定点） / cash（現金残高スナップショット） / intake（取込下書き） / opsTasks（運営委託先のタスク分解と推定時間） / construction（工事資料の所在） / policies（経営方針） / mailFailures（メール処理失敗台帳） / tourismStats（福岡の月次インバウンド定点: stay=外国人延べ宿泊者数(県)・entry=入国者数(福岡空港+博多港)）。" +
...
const READABLE = new Set(["properties", "items", "equipment", "schedules", "jobs", "taxes", "insurance",
  ...
  "assumptions", "scorecards", "vendors", "templates", "alertLogs", "unmatched", "heartbeats", "competitorObs", "cash", "intake", "opsTasks", "construction", "policies", "mailFailures", "tourismStats"]);
