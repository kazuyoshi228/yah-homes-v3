...
### 4.5 checkout のCS・マーケ仕様（2026-08-08決定・8点）

1. **ハウスルール同意（紛争防衛）**: 決済ボタン直前に「□ ハウスルール（パーティー不可・定員厳守・屋外カメラあり）に同意」チェック1つ。**同意日時・UID・文言バージョンをFirestoreに保存**（Stripe紛争・クレーム時の反証材料。§9の紛争分担に証拠として乗る）
2. **マーケ同意opt-in（法対応・初日から）**: 「□ お得な情報・クーポンを受け取る」→ guests に同意フラグ保存。特定電子メール法上、D1クーポン等の販促メールは事前同意が原則（取引通知は例外）。未同意者には取引メールのみ
3. **電話番号=国番号ピッカー＋形式検証**: 言語から国コード初期値を推定（zh-TW→+886等）・桁数検証。滞在中に連絡が取れない事故の防止
4. **宿泊代表者欄（任意・1フィールド）**: 「宿泊代表者名（予約者と異なる場合）」。幹事予約（顧客像の中心）での当日照合・名簿不整合を防ぐ
5. **FAQ3問（畳み表示）**: 駐車場／チェックイン時刻／人数と追加料金。決済直前の不安への先回り。それ以上は/properties/の仕事
6. **予約言語の保存**: UI言語（ja/en/ko/zh/th）を予約レコードに保存（自動メール・CS返信・Beds24インボックスの対応言語判定。partnersApplyと同じ）
7. **完了画面=タイムライン**: 「✅予約確定（予約番号 YH-XXXX）→ 📧3日前に入室案内 → 🔑当日15:00〜」の3ステップ表示＋**カレンダー登録（.ics: 日程・住所・キャンセル期限）**。同内容を確認メールにも。短い予約番号は問い合わせ照合キー
8. **到着予定時刻の収集（予約時には聞かない）**: チェックイン案内メール（3日前・Auto Actions）に「到着予定時刻を教えてください」リンク → /account で選択 → 運営会社へ通知。予約時の摩擦ゼロで運営会社の段取りを改善
...
- **キャンセルセルフサービス**: 7日前まで=ボタン1つ（Stripe自動返金→Beds24取消→カレンダー解放→ゲスト+contact@mail.yah.homes へメール）。以降は「メールで相談」。
- 日程変更は変更依頼フォーム（処理は当面人力・自動化はP2）。
- **到着予定時刻の選択**（§4.5-8・3日前メールから誘導）→ 保存時に運営会社へ通知。
...
**App Check（2026-08-08決定）**: bookingApi・bookCreate に Firebase App Check（reCAPTCHA Enterprise）を必須化。公開エンドポイントのボット濫用（Beds24レート制限の食い潰し・カード試行攻撃）をマネージドで遮断。
