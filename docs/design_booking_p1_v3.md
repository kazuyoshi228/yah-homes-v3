...
> ステータス: **アーカイブ（2026-08-08）**。正本は **design_booking_p1_v4.md**（決定のみ・ミニマル版）。
...
| ~~registry~~ | — | **A案決定により作らない**（名簿は運営会社保有・§9）。B案発動時のみ復活 |
| webhook_events / operations | 禁止 | §8-1/8-2の台帳 |
| audit_logs | 禁止 | 管理操作の改ざん防止ログ（誰が・何を・いつ） |

### 8-5. 管理画面の権限モデル（/admin/bookings）

- Firebase **Custom Claims** で役割定義: `owner`（返金・全操作）／`operator`（閲覧・対応メモ）。付与・剥奪はオーナーのみ。
- **返金上限**: ¥10,000超の返金は確認ダイアログ＋audit_logs必須記録（二者承認はP2）。
- すべての金銭操作を audit_logs に記録。ログにPII・トークンを出さない（マスキング）。
...
| 宿泊者名簿・旅券写し | **運営会社（A案・当社は保有しない）** | — | 当社DBに複製しない |
...
- 通知集約: contact@mail.yah.homes（Stripe紛争・システム通知）。**確認者と確認SLA（24時間以内）・障害メールと顧客メールの振り分けをRunbookに明記**。
- /admin/bookings: §8-5の権限モデル。紛争分担=事実確認は運営会社・Stripe対応はオーナー。エスカレーション権限表は運用開始時に合意。
- **宿泊者名簿（2026-08-08 A案決定・データ最小化）**: 法的義務者=運営会社（許可名義）。**名簿の収集・保有は運営会社が既存フローで行い、当社システムは保有しない**（Airbnbゲストと同じ扱い。直販予約の連絡先はBeds24に載るため、運営会社は既存の収集手段をそのまま適用できる）。当社が保持するPIIは氏名・メール・電話のみに縮小——旅券画像・住所の保管インフラ（registry/Storage）・名簿のメール送付は**作らない**（セキュリティレビューH1/H2はこの決定により解消）。面談確認: 「Airbnbゲストの名簿（住所・外国人旅券確認）は現在どの仕組みで収集しているか。直販ゲストにも同じ運用を適用できるか」→ 収集手段がないと判明した場合のみB案（当社フォーム収集・分離保管・旅券画像は公開ACL絶対禁止/署名付きURL限定）を発動。物件の営業形態・所管自治体の確定は§13-2。
...
| 2 | 物件の営業形態（旅館業/民泊区分）・所管自治体・宿泊税・**名簿の既存収集フローと直販ゲストへの適用可否（A案の成立条件・§9）** | 運営・法務確認 |
