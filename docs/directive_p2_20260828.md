...
- 状態: **全8項目 実装済み（2026-08-28・共有部スレッドが一括消化・発注者指示「全カード分」）**。#5の「表の行tabindex」のみ残（focus-visible・ドットariaは済）
...
| # | 項目 | 担当 | 状態 |
|---|---|---|---|
| 1 | bootData移行（カードごと） | 各カードスレッド | 済 |
| 2 | topbar＋認証UIの共通化 initAuthUI() | 共有部スレッド | 済 |
| 3 | 消し込みの楽観更新 | メンテナンスカード | 済 |
| 4 | ナビ改名＋「準備中」表示 | 共有部スレッド（os-nav.js） | 済 |
| 5 | a11y最低限（focus-visible・行tabindex・ドットaria） | 共有部スレッド（os.css/os-nav.js） | 済 |
| 6 | escの統一（niwaの弱体版是正） | 共有部スレッド | 済 |
| 7 | os.css末尾の上書き群を基底へ畳む | 共有部スレッド | 済 |
| 8 | PROP_LABEL直書き→properties.labelから導出 | 各カードスレッド（overview系APIに同梱） | 済 |
