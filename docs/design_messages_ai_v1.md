...
- 状態: **本番稼働中（2026-08-19・モード=限定自動）**。§6の未決はすべて解消。§10（入室案内の再送）は中止
...
- **必ず人間へ**（auto-limited でも自動送信しない）: 返金・キャンセル・日程/人数変更・クレーム・
  名簿内容・金銭全般・本人確認
- **折り返しの約束を禁止**（2026-08-19 発注者指摘「担当者と連携できていないから一番危ない」）:
  「担当者からご連絡します」型の回答は、折り返す仕組みが無いのに客を待たせるため不可。
  必ず「運営会社へ直接ご連絡ください（電話・メッセージ返信）」とゲスト起点で案内する。
  プロンプト厳守ルール4とQ&A運用メモの両方に明記
...
4. ~~Airstar への説明~~ → **完了（2026-08-19 発注者実施）**。§6の未決はこれで全消化
...
| 2026-08-19 | §10（入室案内の再送）中止 — 名簿提出を機械判定できないため（発注者指摘） |
| 2026-08-19 | Airstarへの説明完了・限定自動で本番稼働開始（§6の未決を全消化） |
| 2026-08-19 | 折り返し約束の禁止（「担当者からご連絡します」不可→運営会社への直接連絡を案内） |
...
## 10. 入室案内の再送 — 案A（2026-08-19 承認 → 同日 **中止**）
...
### 中止（2026-08-19・同日中の判断）

- **理由**: 条件②「名簿提出済み」を機械で判定できない。名簿は Airstar の Zoho フォームに集約され、
  自社の `bookings.registeredAt` は手動フラグ。同日「名簿自社化はしない」と決めたため、
  今後も提出状況は自動で入ってこない。判定できない条件で自動化すると、
  未提出者へ番号を再送する（名簿ゲート＝旅館業法の担保が壊れる）か、提出済みの人を断り続ける
- **代替（現行で十分・すでに稼働中）**: AIが「迷惑メールフォルダもご確認ください。見つからない場合は、
  ご予約いただいたサイトのメッセージ（Airbnb・Booking.com 等／直販は My Page）よりご連絡ください。
  再送いたします」と即答 → 人が名簿提出を確認して再送。ここで人が挟まるのは冗長ではなく
  **名簿ゲートの番人**であり、無理に自動化しない
- **再開の条件**: Zoho Flow からの提出通知 Webhook で `registeredAt` を自動化できたとき
  （Airstar 側の設定が前提）。それが無い限り本節は実装しない
