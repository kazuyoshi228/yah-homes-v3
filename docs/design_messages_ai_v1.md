...
## 2-2. Q&A（チャット・メッセージ共通）— messageInfo は統合により廃止

- **正本は `property_facts/{prop}.chatInfo` の1表のみ**（2026-08-19 発注者判断
  「結局メッセージもChatだから」）。チャットとメッセージAIの両方がこの表を注入する
- 経緯: 当初「顧客メッセージ用情報（messageInfo）」を別表で新設したが、推奨内容を精査した結果
  chatInfo 88行とほぼ全て重複（ゴミ・喫煙・騒音・アーリー・荷物預かり・変更/キャンセル等）。
  2表あると編集のたびに重複が再生産されるため、実装当日に統合して撤去した
- メッセージ特有の行（名簿依頼文面・暗証番号の一次回答・忘れ物・領収書など約14行）は
  chatInfo に分類を足して追記する（追記シートを発注者へ提出）
- 編集画面: /admin/properties/{施設}/#chat「Q&A（チャット・メッセージ共通）」
...
| 2026-08-19 | inquiry（予約前のお問い合わせ）スレッドにも対応（両棟SSoT注入・通知はマジックリンク経路） |
| 2026-08-19 | Q&Aを chatInfo 1表に統合・messageInfo 撤去（「結局メッセージもChatだから」・§2-2） |
| 2026-08-19 | 空室・料金をAIに注入（発注者指示「在庫をAIに注入したい」）: AIが日付を正規化→サーバが bookingRules/checkBookingWindow/quoteFor（Beds24実勢・予約と同一ソース）で照会→結果を注入して確定回答。照会は1通1回・料金の確定額は予約ページ表示と案内 |
...
### P3実装メモ（inquiry対応＋空室・料金注入・2026-08-19 本番デプロイ済み）

- inquiry スレッド: 予約コンテキストの代わりに両棟の【施設情報】＋Q&A（各棟cap縮小）を注入。
  名簿・暗証番号は「ご予約後の流れ」として触れるのみ。自動送信時の通知は notifyInquiry
  （マジックリンク再発行）・Beds24複製は mirrorInquiryToBeds24 ＝既存経路を踏襲
- 空室・料金（2段生成）: 1回目の構造化出力に availabilityNeeded / availabilityQuery
  （prop・checkin・checkout・guests。現在日時JSTを注入して日付を正規化）→ サーバが
  bookingRules（SSoT）→ checkBookingWindow（締切・上限・定員）→ quoteFor（Beds24実勢・
  15分キャッシュ）を決定的に実行 → 【空室・料金】ブロックを注入して2回目で確定回答。
  照会は1メッセージ1回・満室と接続失敗を区別（「満室と言い切らない」既存原則を踏襲）・
  料金は「確定額は予約ページの表示」と必ず案内。ai_drafts に quote（照会内容）を記録
