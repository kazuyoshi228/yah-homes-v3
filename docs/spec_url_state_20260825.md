...
- 状態: **承認済み・共有部ぶん実装済み（2026-08-25）**
  - 実装済み: os-core.js の urlState ／ CLAUDE.md へのキー表転記（手順1）
  - 実装済み（2026-08-25・手順3）: index をクエリルーティングへ移行。?card=properties&tab=ct&prop=kiyokawa の形。
    os-embed.js の復元契約（s=）は不変・旧ハッシュのブックマークは自動でクエリに読み替え
  - 手順2（各カードの配線）: reports.html 済み（?tab=arc で原本アーカイブ直行・2026-08-25）。他カードは各担当スレッドが随時
- **設計変更（2026-08-25夜・発注者決定）: URLは入口専用。** 画面状態→URLの自動反映は全廃した。
  3度の安定化でも書き手（定期スキャン・復元後の整合書き・ページ独自コピー）が残る限り揺れが再発したため。
  - 開いたURLは1文字も書き換えない。復元（深いリンクで開く）は従来どおり動く
  - URLが変わるのは左ナビのカードクリックとロゴだけ
  - 共有はトップバーの「リンクをコピー」——その瞬間の状態からURLを組み立てる（意図した共有）
  - カード内の urlState.set（iframe自身のURL更新）は無害なので存続。標準キー表も入口として有効
- 起票: 2026-08-25
