...
- 状態: **承認済み・実装済み（2026-08-25）**
  - ga4TeitenSync 毎朝8:05 稼働・初回実行で 7/12〜8/24 の44日をバックフィル済み
  - CV突合（ga4Daily vs bookingDaily.cv）を毎朝の点検メールに追加
  - 画面はフル列＋8/16切替線＋7日比較。指標クリックの推移は見送り（表自体が日次時系列のため・予約状況と同判断）
