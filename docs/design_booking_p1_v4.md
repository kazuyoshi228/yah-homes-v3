...
1. **ハウスルール同意**チェック1つ。文言=パーティー/イベント不可・**申告人数厳守（タオル・寝具は申告人数分のみ用意）**・玄関に屋外防犯カメラ・屋内禁煙。同意日時・UID・文言版を保存（紛争時の反証。過少申告の抑止も兼ねる）。
...
8. **到着予定時刻（2026-08-08改定）**: **checkout に任意欄として設置**（15:00〜23:00の1時間刻み＋24:00以降＋未定・初期値=未定）。「セキュリティキーボックスでの受け渡しのため、到着時刻の制限はありません」を併記し、深夜着の不安を予約前に消す。未回答者には3日前メールのリンクから /account で追記してもらう（二段構え）。回答は運営会社へ通知。
...
- 日程変更は依頼フォーム（当面人力）。到着予定時刻の追記・変更（checkoutで未回答/変更したい場合の受け皿）→運営会社通知。
- **入室案内の再表示**（メール不達時の救済）。
